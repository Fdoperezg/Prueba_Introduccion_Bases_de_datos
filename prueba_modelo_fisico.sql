...
CREATE TABLE clientes(
    ...
    rut_cliente INT NOT NULL UNIQUE, direccion_cliente VARCHAR(255), 
...
CREATE TABLE categorias(
    id_categoria SERIAL, 
    nombre_categoria VARCHAR(255) NOT NULL, descripcion_categoria VARCHAR(255), 
...
CREATE TABLE productos(
    id_producto SERIAL, 
    nombre_producto VARCHAR(255) NOT NULL, descripcion_producto VARCHAR(255), valor_unitario FLOAT, 
    id_categoria INT, 
...
SELECT nombre_cliente FROM clientes WHERE id_cliente IN(
    SELECT id_cliente FROM facturas WHERE precio_total=(
        SELECT MAX (precio_total) FROM facturas)
);
--Segunda consulta: ¿Qué cliente pagó sobre 100 de monto?
SELECT nombre_cliente FROM clientes WHERE id_cliente IN(
    SELECT id_cliente FROM facturas WHERE precio_total > 100
);
--Tercera consulta: ¿Cuántos clientes han comprado el producto 6?
SELECT COUNT(DISTINCT id_cliente) FROM facturas WHERE numero_factura IN(
    SELECT numero_factura FROM facturas_productos WHERE id_producto=6
);